...
{
	.name = "imx-pcm186x",
	.stream_name = "imx-pcm186x",
	.codec_dai_name = "pcm1863-aif",
...
static struct snd_soc_card snd_soc_card_pcm186x = {
	.name         = "imx-audio-pcm186x",
	.owner        = THIS_MODULE,
	.dai_link     = imx_dai,
...
{
	struct snd_soc_card *card = &snd_soc_card_pcm186x;	
...
static int imx_pcm186x_remove(struct platform_device *pdev)
{

	return snd_soc_unregister_card(&snd_soc_card_pcm186x);
...
static struct platform_driver imx_pcm186x_driver = {
	...
	.remove = imx_pcm186x_remove,
};

module_platform_driver(imx_pcm186x_driver);

/* Module information */
MODULE_DESCRIPTION("ALSA SoC i.MX SAI for pcm186x");
